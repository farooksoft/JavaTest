...
/* 
Change campaign id from "source" to "cid".
Remove old form abandonment code.
Clean up pilot residue.
pagetype/404 fix
Add opensolaris.org and sun-catalogue.com to internal links.
*/
...
var s_linkInternalFilters="javascript:,sun.com,java.com,sunsolve,star-auth,sfbay,amused.corp,opensolaris.org,sun-catalogue.com"
...
/* Populate "s_channel" variable (if not already populated). Based on 8 levels of directory. */
if(s_channel==""){
  var s_t1=0
  var s_t2=0
  var s_du=""
  ...
  if((s_t1=s_du.indexOf("://"))>=0){
    ...
    // Remove everything after the eigth "/"
    s_t1=0
    for(s_t2=0;s_t2<s_channel.length;s_t2++){
      if(s_channel.charAt(s_t2)==":"){
        s_t1++
...
/* STANDARD SUN GLOBAL ACCOUNT CODE: pre-pend "pageName" and "channel" with "s_siteid" */
