...
public class OrbitCreateRoom extends Dialog implements ActionListener,
	...
	OrbitCreateRoom(Frame parent) {
		...
		passwordWarningLabel1 = new Label(
				"Warning: Your Java client cannot encrypt your");
		...
		passwordWarningLabel2 = new Label(
				"passwords.  They will be sent as plain text.");
	...
	protected void goCreate() {
		if (!roomName.getText().equals("")) {
			String pword = "";
			if (priv.getState())
				pword = parentWindow.passwordEncryptor.encryptPassword(password
						.getText());

			parentWindow.canvas.clear();

			try {
				parentWindow.theClient.sendEnterRoom(roomName.getText(),
						priv.getState(), pword);
			} catch (IOException e) {
