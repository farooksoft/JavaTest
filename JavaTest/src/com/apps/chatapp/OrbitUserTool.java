...
public class OrbitUserTool {
	...
	private void readPasswordFile() {
		...
		try {
			passwordStream = new DataInputStream(new FileInputStream(
					OrbitServer.userPasswordFileName));
			...
			while (true) {
				...
				// the vector
				users.addElement(new OrbitUser(0, tempUserName, tempPassword,
						""));
			}
		} catch (IOException e) {
		}
	...
	private void writePasswordFile() throws Exception {
		...
		try {
			passwordStream = new DataOutputStream(new FileOutputStream(
					OrbitServer.userPasswordFileName));
	...
	private void appendUser(String userName, String encryptedPassword)
		...
		try {
			passwordStream = new DataOutputStream(new FileOutputStream(
					OrbitServer.userPasswordFileName, true));
		...
		for (int count = 0; count < args.length; count++) {
			if (args[count].equals("-create")) {
				...
				try {
					...
					System.out.println("Error listing users: " + e.toString());
					System.exit(1);
				}

				// Print out the results
				for (int count2 = 0; count2 < userList.length; count2++)
				...
				System.out.println("\nOrbitUserTool: unknown " + "argument "
						+ args[count]);
