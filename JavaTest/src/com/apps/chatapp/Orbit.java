...
public class Orbit extends Object implements Runnable {
	...
	private void usage() {
		...
		System.out.println("java Orbit [" + usernameParam + " name] ["
				+ passwordParam + " password] [" + servernameParam + " host] ["
				+ portnumberParam + " port] [" + chatroomParam + " room] ["
				+ widthParam + " number] [" + heightParam + " number] ["
				+ nopasswordsParam + "] [" + locksettingsParam + "] ["
				+ autoconnectParam + "] [" + hidecanvasParam + "]");
		return;
	}
	...
	private boolean parseArgs(String[] args) {
		...
		for (int count = 0; count < args.length; count++) {
			if (args[count].equals(usernameParam)) {
				...
				System.out.println("\nOrbit: unknown argument " + args[count]);
	...
	public Orbit(String[] args) {
		...
		// Open the window
		window = new OrbitWindow(name, password, host, port, showCanvas, myURL);
