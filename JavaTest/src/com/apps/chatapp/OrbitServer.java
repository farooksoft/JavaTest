...
class OrbitServerShutdown extends Thread {
...
public class OrbitServer extends Thread {
	...
	public OrbitServer(int askport, boolean passwords, boolean newusers,
		...
		if (graphics) {
			myWindow = new OrbitServerWindow(this, "Intranet Chatting v"
					+ Orbit.VERSION + " Server");
		...
		try {
			...
			serverOutput("Unable to start chat log for room " + mainRoom.name
					+ "\n");
		}
		...
		// out this bit.
		float javaVersion = Float.parseFloat(System.getProperty("java.version")
				.substring(0, 3));
		if (javaVersion >= 1.3)
			Runtime.getRuntime().addShutdownHook(new OrbitServerShutdown(this));
	...
	public boolean checkPassword(String fileName, String userName,
		...
		try {
			...
			passwordStream = new DataInputStream(new FileInputStream(fileName));
	...
	protected OrbitChatRoom findChatRoom(String roomName) {
		...
		if (mainRoom.name.equals(roomName))
			...
			for (int count = 0; count < chatRooms.size(); count++) {
				tempRoom = (OrbitChatRoom) chatRooms.elementAt(count);
	...
	public boolean isUserAllowed(OrbitChatRoom room, OrbitClientSocket client,
			String password) {
		...
		if (room.priv) {
			...
			if (!room.password.equals(password)) {
				...
				for (int count2 = 0; count2 < room.invitedUsers.size(); count2++) {
					Integer Id = (Integer) room.invitedUsers.elementAt(count2);
				...
				if (!invited) {
					try {
						client.sendServerMessage("Not invited to/incorrect password "
								+ "for the private room " + room.name);
					} catch (IOException e) {
		...
		for (int count1 = 0; count1 < room.bannedUserNames.size(); count1++) {
			if (room.bannedUserNames.elementAt(count1).equals(client.user.name)) {
	...
	public synchronized void disconnect(OrbitClientSocket who, boolean notify) {
		int count;
		...
		serverOutput("There are " + currentConnections + " users connected\n");

		// Tell all the other clients to ditch this user
		for (count = 0; count < currentConnections; count++) {
			OrbitClientSocket other = (OrbitClientSocket) connections
					.elementAt(count);
			try {
		...
		if (graphics) {
			synchronized (myWindow.userList) {
				...
				// one disconnection.
				for (count = 0; count < myWindow.userList.getItemCount(); count++) {
					if (myWindow.userList.getItem(count).equals(who.user.name)) {
	...
	public synchronized void disconnectAll(boolean notify) {
		...
		for (count = (currentConnections - 1); count >= 0; count--) {
			OrbitClientSocket temp = (OrbitClientSocket) connections
					.elementAt(count);
	...
	public void run() {
		while (!stop) {
			...
			OrbitClientSocket cs = new OrbitClientSocket(this, mySocket,
					myThreadGroup);
		}
	}
	...
	protected void readMessages() {
		...
		try {
			messageStream = new DataInputStream(new FileInputStream(
					messageFileName));
	...
	protected void saveMessages() {
		...
		try {
			messageStream = new DataOutputStream(new FileOutputStream(
					messageFileName));

			for (int count = 0; count < messages.size(); count++) {
				OrbitMessage tempMessage = (OrbitMessage) messages
						.elementAt(count);
	...
	protected void shutdown() {
		...
		if (currentConnections > 0) {
			...
			for (int count = 0; count < currentConnections; count++) {
				OrbitClientSocket who = (OrbitClientSocket) connections
						.elementAt(count);
				try {
					who.sendDisconnect(who.user.id,
							"The server is shutting down.  " + "Goodbye.");
				} catch (IOException e) {
				}
			}
			// Make sure
		...
		// Print some stats
		serverOutput("Peak connections this session: " + peakConnections + "\n");
		serverOutput("Total connections this session: " + totalConnections
				+ "\n");
	...
	private static void usage() {
		...
		System.out.println("java OrbitServer [" + portnumberParam
				+ " number] [" + usepasswordsParam + "] [" + newusersParam
				+ "] [" + nographicsParam + "] [" + chatlogsParam + "]");
		return;
	}
				...
				if (++count < args.length) {
					try {
						...
						System.out.println("\nIntranet Chatting Server: "
								+ "illegal port number " + args[count]);
