...
public class OrbitUserToolDialog extends Dialog implements ActionListener,
	...
	private void createUser() {
		...
		try {
			userTool.createUser(name.getText(), encryptedPassword);
		} catch (Exception e) {
			statusLabel.setText("Error creating user");
			new OrbitInfoDialog(parentWindow, "Error creating user", true,
					e.toString());
			return;
		}
	...
	private void deleteUsers() {
		...
		for (int count = 0; count < users.length; count++) {
			try {
				...
				new OrbitInfoDialog(parentWindow, "Error deleting user", true,
						e.toString());
				return;
			}
		}
	...
	public void keyReleased(KeyEvent E) {
		if (E.getKeyCode() == E.VK_ENTER) {
			...
			else if ((E.getSource() == name) || (E.getSource() == password)) {
				createUser();
				return;
			}
		}
