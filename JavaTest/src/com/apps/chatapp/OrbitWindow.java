...
public class OrbitWindow extends Frame implements ActionListener, ItemListener,
		KeyListener, MouseListener, WindowListener, ClipboardOwner {
	public int id = 0;
	public String name = "";
	...
	public OrbitWindow(String userName, String userPassword, String hostName,
			String portName, boolean isCanvas, URL myURL) {
		super();
		...
		drawCanvasLabel = new Label("Drawing canvas:");
		myConstraints.gridx = 0;
		...
		try {
			URL imageUrl = new URL(OrbitURL.getProtocol(), OrbitURL.getHost(),
	...
	public void setIcon() {
		...
		try {
			URL iconUrl = new URL(OrbitURL.getProtocol(), OrbitURL.getHost(),
					OrbitURL.getFile() + "OrbitIcon.gif");
	...
	public void online() {
		...
		menuChatRooms.setEnabled(true);
		page.setEnabled(true);
		menuMessaging.setEnabled(true);
		messaging.setEnabled(true);
		menuPage.setEnabled(true);
		if (adminConsole)
			roomOwner(true);
		setTitle("Intranet Chatting v" + Orbit.VERSION + " - online at " + host);
		return;
	}
	...
	public void offline() {
		...
		roomOwner(false);
		setTitle("Intranet Chatting v" + Orbit.VERSION + " - offline");
		return;
	}
	...
	protected void connect() {
		...
		if (!haveRequiredInformation()) {
			...
			// Do we have it NOW?
			if (!haveRequiredInformation()) {
				new OrbitInfoDialog(this, "Connection canceled", true,
		...
		try {
			...
			new OrbitInfoDialog(this, "Couldn't connect", true,
					"Couldn't connect to port " + portNumber + " on server "
							+ host);
			return;
		} catch (Exception c) {
			...
			String errorString = "Java exception while attempting connection:\n";
			...
			new OrbitTextDialog(this, "Couldn't connect", errorString, 60, 15,
	...
	protected void showUserInfo() {
		...
		for (int count1 = 0; count1 < selectedUsers.length; count1++) {
			...
			for (int count2 = 0; count2 < theClient.userList.size(); count2++) {
				OrbitUser tmp = (OrbitUser) theClient.userList
						.elementAt(count2);
				...
				if (selectedUsers[count1].equals(tmp.name)) {
					...
					new OrbitTextDialog(this, "User information for "
							+ tmp.name, message, 40, 10,
	...
	protected void saveText() {
		...
		FileDialog saveTextDialog = new FileDialog(this,
				"Save chat text as...", FileDialog.SAVE);
		saveTextDialog.show();
	...
	protected void pastePictureFile() {
		...
		FileDialog getPictureDialog = new FileDialog(this,
				"Paste picture to whiteboard...", FileDialog.LOAD);
		...
		if (pictureFile == null) {
			// ???
			new OrbitInfoDialog(this, "Failed", true, "Can't open that file");
			return;
		}
	...
	public void actionPerformed(ActionEvent E) {
		...
		if (E.getSource() == menuManual) {
			try {
				URL manualURL = new URL(OrbitURL.getProtocol(),
						OrbitURL.getHost(), OrbitURL.getFile() + "MANUAL.TXT");
				...
				new OrbitTextDialog(this, "Orbit Online Manual", input, 65, 25,
						TextArea.SCROLLBARS_VERTICAL_ONLY, false);
				return;
			} catch (IOException G) {
				new OrbitInfoDialog(this, "Data not available", true,
		...
		if (E.getSource() == menuAbout) {
			...
			new OrbitTextDialog(this, "About Intranet Chatting", abouttext, 60,
					22, TextArea.SCROLLBARS_NONE, false);
			return;
		}
		...
		if (E.getSource() == typed) {
			...
			if (E.getKeyCode() == E.VK_ENTER) {
				...
				if (!sendToAll.getState()) {
					...
					for (int count = 0; count < wholist.length; count++) {
				...
				if (connected == true)
					try {
						...
						theClient.sendActivity(OrbitCommand.ACTIVITY_TYPING);
					} catch (IOException e) {
						theClient.lostConnection();
						return;
					}
	...
	public void itemStateChanged(ItemEvent E) {
		...
		if (E.getSource() == colorChoice) {
			canvas.drawColor = canvas.colourArray[colorChoice
					.getSelectedIndex()];
			return;
		}
