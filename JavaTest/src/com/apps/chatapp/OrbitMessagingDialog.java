...
public class OrbitMessagingDialog extends Dialog implements ActionListener,
	...
	public OrbitMessagingDialog(OrbitWindow parent) {
		...
		messageText = new TextArea("", 2, 20, TextArea.SCROLLBARS_VERTICAL_ONLY);
		...
		if (!saveUser.equals(""))
			...
			for (int count = 0; count < usersVector.size(); count++) {
				...
				if (user.name.equals(instantUser)) {
					try {
						parentWindow.theClient.sendInstantMess(user.id,
								messageText.getText());
					} catch (IOException e) {
						parentWindow.theClient.lostConnection();
						return;
	...
	public void keyReleased(KeyEvent E) {
		if (E.getKeyCode() == E.VK_ENTER) {
			...
			else if ((E.getSource() == ok) || (E.getSource() == saveFor)
