...
public class OrbitRoomsDialog extends Dialog implements ActionListener,
	...
	public void receivedList() {
		...
		synchronized (roomList) {
			...
			for (int count = 0; count < parentWindow.roomInfoArray.length; count++)
	...
	protected void goEnter() {
		...
		if (index >= 0) {
			...
			if (parentWindow.currentRoom == selectedRoom) {
				new OrbitInfoDialog(parentWindow, "Change unnecessary", true,
						"You are already in the room \"" + selectedRoom.name
								+ "\"");
				return;
			}
			...
			if (selectedRoom.priv && !selectedRoom.invited) {
				OrbitPasswordDialog passDialog = new OrbitPasswordDialog(
						parentWindow, "Enter password for private room \""
	...
	protected void showInfo() {
		...
		if (index >= 0) {
			...
			for (int count = 0; count < selectedRoom.userNames.size(); count++) {
				infoString += (String) selectedRoom.userNames.elementAt(count);
			...
			new OrbitTextDialog(parentWindow, "Chat room information",
					infoString, 30, 10, TextArea.SCROLLBARS_VERTICAL_ONLY,
					false);
		}
	}
	...
	public void itemStateChanged(ItemEvent E) {
		if (E.getSource() == roomList) {
			...
			boolean somethingSelected = (roomList.getSelectedItems().length > 0);
			...
			// bits.
			OrbitRoomInfo selectedRoom = parentWindow.roomInfoArray[roomList
					.getSelectedIndex()];
		}
	}
