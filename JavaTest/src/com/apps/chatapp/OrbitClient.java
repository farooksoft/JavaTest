...
public class OrbitClient extends Thread {
	...
	public OrbitClient(String host, String name, int portnumber,
			OrbitWindow mainWindow) throws UnknownHostException, IOException,
			Exception {
	...
	void parseCommand() throws IOException {
		...
		synchronized (istream) {
			...
			if (protocolVersion >= 2.0) {
				...
				default: {
					...
					System.out
							.println("client: unknown command " + commandType);
					break;
				}
				}
			}
	...
	protected OrbitUser readUser() throws IOException {
		...
		if (parentWindow.sendToAll.getState()) {
			...
			for (int count1 = 0; count1 < numberUsers; count1++) {
				...
				for (int count2 = 0; count2 < userList.size(); count2++) {
					OrbitUser tmp = (OrbitUser) userList.elementAt(count2);
	...
	protected void sendProtocol(double version) throws IOException {
	...
	protected void receiveProtocol() throws IOException {
	...
	protected void receiveConnect() throws IOException {
		...
		parentWindow.messages.append("<<New user \"" + userName
				+ "\" connected>>\n");
	}

	protected void sendUserInfo() throws IOException {
		...
		synchronized (ostream) {
			...
			ostream.writeBoolean(parentWindow.passwordEncryptor.canEncrypt);
	...
	protected void receiveServerMess() throws IOException {
		String message = "";
		...
		new OrbitInfoDialog(parentWindow, "Server message", true, message);
	}
	...
	protected void receiveDisconnect() throws IOException {
		...
		if ((tmpId == parentWindow.id) || (tmpId == 0)) {
			...
			synchronized (list) {
				for (int count2 = 0; count2 < list.getItemCount(); count2++) {
			...
			if (parentWindow.roomInfoArray != null) {
				for (int count2 = 0; count2 < parentWindow.roomInfoArray.length; count2++)
					// Is this the user's chat room?
					if (parentWindow.roomInfoArray[count2].name.equals(tmpUser
							.getChatRoomName())) {
						roomInfo = parentWindow.roomInfoArray[count2];
						break;
					}
				if (roomInfo != null) {
					// Remove the user from the room's list
					// of users
					roomInfo.userNames.removeElement(tmpUser.name);
	...
	protected void requestRoomList() throws IOException {
	...
	protected void receiveInvite() throws IOException {
		...
		new OrbitInfoDialog(parentWindow, "Invitation", false,
				"You have been invited to join the room \"" + roomName
						+ "\" by user " + fromUser.name);
	}
	...
	protected void receiveEnterRoom() throws IOException {
		...
		if (userId == parentWindow.id) {
			...
			for (int count1 = 0; count1 < parentWindow.roomInfoArray.length; count1++) {
				OrbitRoomInfo roomInfo = parentWindow.roomInfoArray[count1];
			...
			if (parentWindow.currentRoom.creatorName.equals(parentWindow.name))
				parentWindow.roomControlDialog = new OrbitRoomControl(
						parentWindow);
		...
		for (int count1 = 0; count1 < userList.size(); count1++) {
			...
			if (oldRoomName.equals(parentWindow.currentRoom.name)) {
				if (!oldRoomName.equals(""))
					parentWindow.messages
							.append("<<" + tmpUser.name
									+ " moved to chat room \"" + newRoomName
									+ "\">>\n");
	...
	protected void receiveBootUser() throws IOException {
		...
		new OrbitInfoDialog(parentWindow, "Booted", true,
				"You have been booted from the room \"" + roomName
						+ "\" by user " + fromUser.name);
	}
	...
	protected void receiveBanUser() throws IOException {
		...
		new OrbitInfoDialog(parentWindow, "Banned", true,
				"You have been banned from the room \"" + roomName + "\"");
	}
	...
	protected void receiveAllowUser() throws IOException {
		...
		new OrbitInfoDialog(parentWindow, "Allowed", true,
				"You are now allowed to join the room \"" + roomName + "\"");
	}
	...
	protected void sendLine(short colour, short startx, short starty,
			short endx, short endy, short thick) throws IOException {
	...
	protected void receiveLine() throws IOException {
		...
		parentWindow.canvas.drawLine(colour, startx, starty, endx, endy, thick,
				OrbitCanvas.MODE_PAINT);
	}
	...
	protected void sendDrawText(short colour, short x, short y, short type,
			short attribs, short size, String text) throws IOException {
	...
	protected void sendClearCanv() throws IOException {
	...
	protected void sendPageUser() throws IOException {
		...
		// Otherwise, list the users that have been paged
			...
			if (numberUsers == 1)
				parentWindow.messages.append("<<paging " + selectedUsers[0]
						+ ">>\n");
			else {
				...
				for (int count = 0; count < numberUsers; count++)
					parentWindow.messages.append(" " + selectedUsers[count]);
				parentWindow.messages.append(">>\n");
			}
		}
	}
	...
	protected void receivePageUser() throws IOException {
		...
		if (parentWindow.menuPlaySound.getState()) {
			...
			// this sound
			float javaVersion = Float.parseFloat(System.getProperty(
			...
			if (javaVersion < 1.2)
				...
				try {
					URL soundURL = new URL(parentWindow.OrbitURL.getProtocol(),
							parentWindow.OrbitURL.getHost(),
							parentWindow.OrbitURL.getFile() + "OrbitPage.au");
	...
	public void receiveInstantMess() throws IOException {
		OrbitUser fromUser = null;
		String message = "";
	...
	public void receiveStoredMess() throws IOException {
		...
		for (int count = 0; count < numberMessages; count++) {
			...
			// Place it on the screen as a dialog box
			new OrbitTextDialog(parentWindow, "Message from " + from, message,
					40, 10, TextArea.SCROLLBARS_VERTICAL_ONLY, true);
		}
	...
	public synchronized void shutdown(boolean notifyUser) {
		...
		if (notifyUser)
			...
			new OrbitInfoDialog(parentWindow, "Disconnected", true,
					("Disconnected from " + parentWindow.host));
