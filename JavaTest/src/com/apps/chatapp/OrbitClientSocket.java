...
class OrbitCanvasCommand {
	...
	OrbitCanvasCommand(short mycommandtype, int mywhodrew, short mycolour,
			short myx1, short myy1, short myx2, short myy2, short mytype,
			short myattribs, short mysize, short mythick, boolean myfill,
			String mytext, byte[] mypicturedata) {
...
class OrbitClientTickler extends Thread {
...
public class OrbitClientSocket extends Thread {
	...
	protected OrbitClientSocket findClientSocket(int userid) {
		...
		for (int count = 0; count < server.currentConnections; count++) {
			tempClient = (OrbitClientSocket) server.connections
					.elementAt(count);
	...
	protected void enterChatRoom(OrbitChatRoom room) throws IOException {
		...
		if (chatRoom != room) {
			...
			for (int count = 0; count < server.currentConnections; count++) {
				tmpClient = (OrbitClientSocket) server.connections
						.elementAt(count);
		...
		for (int count = 0; count < chatRoom.canvasHistory.size(); count++) {
			...
			try {
				command = (OrbitCanvasCommand) chatRoom.canvasHistory
						.elementAt(count);
			...
			case OrbitCommand.LINE: {
				sendLine(command.whodrew, command.colour, command.x1,
						command.y1, command.x2, command.y2, command.thick);
				break;
			}
			case OrbitCommand.RECT:
			case OrbitCommand.OVAL: {
				sendPoly(command.commandtype, command.whodrew, command.colour,
						command.x1, command.y1, command.x2, command.y2,
						command.thick, command.fill);
				break;
			}
			case OrbitCommand.DRAWTEXT: {
				sendDrawText(command.whodrew, command.colour, command.x1,
						command.y1, command.type, command.attribs,
						command.size, command.text);
				break;
			}
			case OrbitCommand.DRAWPICTURE: {
				sendDrawPicture(command.whodrew, command.x1, command.y1,
						command.pictureData);
				break;
			}
			}
	...
	protected void leaveChatRoom() throws IOException {
	...
	protected void negotiateProtocol() throws IOException {
		double preferred = 0;
	...
	protected void sendUserInfo(OrbitUser newuser) throws IOException {
	...
	protected void receiveUserInfo() throws IOException {
		OrbitClientSocket tmpClient;
		...
		if (!istream.readBoolean())
			user.password = server.passwordEncryptor
		...
		if (this.user.name.equals("Administrator")) {
			...
			if (!tmpAddr.getHostName().equals("localhost")
				...
				sendDisconnect(0, "The user name \"Administrator\" "
						+ "is reserved for the server console " + "user");
				server.serverOutput("User from " + tmpAddr.getHostName()
						+ " attempted to connect " + "as an administrator\n");
		...
		for (int count = 0; count < server.currentConnections; count++) {
			OrbitClientSocket tmpSocket = ((OrbitClientSocket) server.connections
					.elementAt(count));

			if (tmpSocket.user.name.equals(user.name)) {
				System.out.println("Sending 'already' message");
				sendDisconnect(0, "There is already a user " + "called \""
						+ user.name + "\" logged in!");
		...
		if (server.requirePasswords) {
			...
			try {
				authenticated = server.checkPassword(
						OrbitServer.userPasswordFileName, user.name,
						user.password);
			} catch (Exception e) {
				...
				if (server.allowNewUsers) {
					...
					try {
						server.createNewUser(user.name, user.password);
					} catch (Exception f) {
			...
			if (!authenticated) {
				...
				sendDisconnect(0, "Incorrect user name or " + "password");
				InetAddress tmpAddr = mySocket.getInetAddress();
				server.serverOutput("Failed login for user name \"" + user.name
						+ "\" from host " + tmpAddr.getHostName() + "\n");
		...
		for (int count = 0; count < server.currentConnections; count++) {
			tmpClient = (OrbitClientSocket) server.connections.elementAt(count);

			if (tmpClient.user != user) {
				sendUserInfo(tmpClient.user);
				sendEnterRoom(tmpClient.user.id, tmpClient.user.getChatRoom());
		...
		try {
			URL welcomeURL = new URL(server.myURL.getProtocol(),
					server.myURL.getHost(), server.myURL.getFile()
							+ OrbitServer.welcomeMessageName);
		...
		if (server.currentConnections == 1)
			...
			mess = "There are " + (server.currentConnections - 1)
					+ " other users online";
			if (server.chatRooms.size() > 0)
				mess += " in " + (server.chatRooms.size() + 1) + " chat rooms";
		} else {
		...
		for (int count = 0; count < server.messages.size(); count++)
			if (((OrbitMessage) server.messages.elementAt(count)).messageFor
					.equals(user.name))
				numberMessages++;
		if (numberMessages > 0)
	...
	protected void sendServerMessage(String data) throws IOException {
	...
	protected void sendDisconnect(int userId, String mess) throws IOException {
	...
	protected void sendRoomList() throws IOException {
		OrbitClientSocket tmpClient;
		...
		synchronized (ostream) {
			...
			for (int count1 = 0; count1 < server.mainRoom.clients.size(); count1++) {
				tmpClient = (OrbitClientSocket) server.mainRoom.clients
						.elementAt(count1);
			...
			for (int count1 = 0; count1 < server.chatRooms.size(); count1++) {
				OrbitChatRoom room = (OrbitChatRoom) server.chatRooms
						.elementAt(count1);
				...
				if (room.priv) {
					...
					for (int count2 = 0; count2 < room.invitedUsers.size(); count2++)
						if (((Integer) room.invitedUsers.elementAt(count2))
								.intValue() == user.id) {
							invited = true;
							break;
						}
				...
				// chat room
				for (int count2 = 0; count2 < room.clients.size(); count2++) {
					tmpClient = (OrbitClientSocket) room.clients
							.elementAt(count2);
	...
	protected void receiveRoomList() throws IOException {
	...
	protected void sendInvite(int userId, String roomName) throws IOException {
	...
	protected void receiveInvite() throws IOException {
	...
	protected void receiveEnterRoom() throws IOException {
		...
		if (newRoom == null) {
			...
			newRoom = new OrbitChatRoom(newRoomName, user.name, priv, password);
			try {
				newRoom.setLogging(server.logChats);
			} catch (IOException e) {
				server.serverOutput("Unable to start chat log for " + "room "
						+ newRoom.name + "\n");
			}
	...
	protected void sendBootUser(int userId, String roomName) throws IOException {
	...
	protected void receiveBootUser() throws IOException {
	...
	protected void sendBanUser(int userId, String roomName) throws IOException {
	...
	protected void receiveBanUser() throws IOException {
	...
	protected void receiveAllowUser() throws IOException {
		...
		for (int count = 0; count < allowRoom.bannedUserNames.size(); count++) {
			if (allowRoom.bannedUserNames.elementAt(count).equals(
					allowUser.user.name)) {
	...
	protected void sendActivity(int fromid, short activity) throws IOException {
	...
	protected void receiveActivity() throws IOException {
		...
		if (numRecipients == 0) {
			for (int count = 0; count < chatRoom.clients.size(); count++) {
				toUser = (OrbitClientSocket) chatRoom.clients.elementAt(count);
				if (toUser != this)
	...
	protected void receiveChatText() throws IOException {
	...
	protected void receiveLine() throws IOException {
		...
		if (numRecipients == 0) {
			for (int count = 0; count < chatRoom.clients.size(); count++) {
				toUser = (OrbitClientSocket) chatRoom.clients.elementAt(count);
				if (toUser != this)
					toUser.sendLine(user.id, colour, startx, starty, endx,
							endy, thick);
			}
		} else {
		...
		chatRoom.canvasHistory.addElement(new OrbitCanvasCommand(
				OrbitCommand.LINE, user.id, colour, startx, starty, endx, endy,
				(short) 0, (short) 0, (short) 0, thick, false, null, null));
	}
	...
	protected void receivePoly(short kind) throws IOException {
		...
		if (numRecipients == 0) {
			for (int count = 0; count < chatRoom.clients.size(); count++) {
				toUser = (OrbitClientSocket) chatRoom.clients.elementAt(count);
				if (toUser != this)
					toUser.sendPoly(kind, user.id, colour, x, y, width, height,
							thick, fill);
			}
		} else {
			// Now loop for each of the intended recipients
			for (int count = 0; count < numRecipients; count++) {
				...
				if (toUser != null)
					if (toUser != this)
						// Send it out to this recipient
						toUser.sendPoly(kind, user.id, colour, x, y, width,
								height, thick, fill);
			}
		}

		// Add this shape to our canvas vector
		chatRoom.canvasHistory.addElement(new OrbitCanvasCommand(kind, user.id,
				colour, x, y, width, height, (short) 0, (short) 0, (short) 0,
				thick, fill, null, null));
	}

	protected void sendDrawText(int fromid, short colour, short x, short y,
			short type, short attribs, short size, String text)
			throws IOException {
	...
	protected void receiveDrawText() throws IOException {
		...
		if (numRecipients == 0) {
			for (int count = 0; count < chatRoom.clients.size(); count++) {
				toUser = (OrbitClientSocket) chatRoom.clients.elementAt(count);
				if (toUser != this)
					toUser.sendDrawText(user.id, colour, x, y, type, attribs,
							size, text);
			}
		} else {
			// Now loop for each of the intended recipients
		...
		chatRoom.canvasHistory.addElement(new OrbitCanvasCommand(
				OrbitCommand.DRAWTEXT, user.id, colour, x, y, (short) 0,
				(short) 0, type, attribs, size, (short) 0, false, text, null));
	}

	protected void sendDrawPicture(int fromid, short x, short y,
			byte[] pictureData) throws IOException {
	...
	protected void receiveDrawPicture() throws IOException {
		...
		chatRoom.canvasHistory.addElement(new OrbitCanvasCommand(
				OrbitCommand.DRAWPICTURE, user.id, (short) 0, x, y, (short) 0,
				(short) 0, (short) 0, (short) 0, (short) 0, (short) 0, false,
				null, data));
	}

	protected void sendClearCanv(int fromid) throws IOException {
	...
	protected void sendPageUser(int fromid) throws IOException {
	...
	protected void receivePageUser() throws IOException {
	...
	protected void receiveInstantMess() throws IOException {
		int whoFor = 0;
		String message = "";
		...
		for (int count = 0; count < server.currentConnections; count++) {
			OrbitClientSocket toUser = (OrbitClientSocket) server.connections
					.elementAt(count);
	...
	protected void receiveLeaveMess() throws IOException {
		String whofor = "";
		String message = "";
		...
		server.messages
				.addElement(new OrbitMessage(whofor, user.name, message));
	}

	protected void receiveReadMess() throws IOException {
	...
	protected void sendStoredMess() throws IOException {
		...
		synchronized (ostream) {
			...
			for (int count = 0; count < server.messages.size(); count++) {
				message = (OrbitMessage) server.messages.elementAt(count);
	...
	protected void sendError(int fromid, short errorCode) throws IOException {
	...
	protected void receiveError() throws IOException {
	...
	protected void sendServerText(String data) throws IOException {
