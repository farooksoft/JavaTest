...
public class OrbitServerWindow extends Frame implements ActionListener,
	...
	public OrbitServerWindow(OrbitServer parent, String Name) {
		...
		stats = new TextField("Connections - current: 0  peak: 0  total: 0", 40);
	...
	public void updateStats() {
		...
		// the server window
		stats.setText("Connections - current: " + myParent.currentConnections
				+ "  peak: " + myParent.peakConnections + "  total: "
	...
	public void actionPerformed(ActionEvent E) {
		if (E.getSource() == userAdmin) {
			OrbitUserToolDialog userTool = new OrbitUserToolDialog(this);
			return;
		}

		if (E.getSource() == console) {
			OrbitInfoDialog tmp = new OrbitInfoDialog(this, "Loading", false,
					"Starting the client, one moment please...");
			consoleWindow = new OrbitWindow("Administrator", "", "localhost",
					Integer.toString(myParent.port), true, myParent.myURL);
			tmp.dispose();
		...
		if (E.getSource() == disconnect) {
			...
			if (disconnectUser != null) {
				...
				for (int count = 0; count < myParent.currentConnections; count++) {
					OrbitClientSocket tempuser = (OrbitClientSocket) myParent.connections
							.elementAt(count);
	...
	public void itemStateChanged(ItemEvent E) {
		...
		if (E.getSource() == logChat) {
			...
			for (int count = 0; count < myParent.chatRooms.size(); count++) {
				OrbitChatRoom tmp = (OrbitChatRoom) myParent.chatRooms
						.elementAt(count);
				try {
					tmp.setLogging(myParent.logChats);
				} catch (IOException e) {
					myParent.serverOutput("Unable to toggle logging "
...
class OrbitServerShutdownDialog extends Dialog implements ActionListener,
	...
	public OrbitServerShutdownDialog(OrbitServerWindow serverwindow,
		...
		setLocation(
				(((((myParent.getBounds()).width) - ((getSize()).width)) / 2) + ((myParent
						.getLocation()).x)),
				(((((myParent.getBounds()).height) - ((getSize()).height)) / 2) + ((myParent
						.getLocation()).y)));
...
class OrbitPictureCanvas extends Canvas {
