...
class OrbitLine {
	...
	public OrbitLine(Color mycolor, int mystartx, int mystarty, int myendx,
			int myendy, int mythickness) {
		color = mycolor;
		startx = mystartx;
		starty = mystarty;
		endx = myendx;
...
class OrbitSampleCanvas extends Canvas {
	...
	public void paint(Graphics g) {
		...
		g.setFont(new Font(OrbitCanvas.fontArray[parentWindow.drawFontNumber],
...
class OrbitFontSelect extends Dialog implements ActionListener, ItemListener,
	...
	public OrbitFontSelect(Frame parent) {
		...
		size.addItem("72");
		size.addItem("120");
		if (parentWindow.drawSize == 10)
			size.select(0);
		if (parentWindow.drawSize == 12)
			size.select(1);
		if (parentWindow.drawSize == 14)
			size.select(2);
		if (parentWindow.drawSize == 18)
			size.select(3);
		if (parentWindow.drawSize == 20)
			size.select(4);
		if (parentWindow.drawSize == 24)
			size.select(5);
		if (parentWindow.drawSize == 36)
			size.select(6);
		if (parentWindow.drawSize == 48)
			size.select(7);
		if (parentWindow.drawSize == 60)
			size.select(8);
		if (parentWindow.drawSize == 72)
			size.select(9);
		if (parentWindow.drawSize == 120)
			size.select(10);
		myConstraints.gridx = 1;
		myConstraints.gridy = 1;
		myConstraints.gridheight = 1;
		...
		pack();
		setLocation(
				(((((parentWindow.getBounds()).width) - ((getSize()).width)) / 2) + ((parentWindow
						.getLocation()).x)),
				(((((parentWindow.getBounds()).height) - ((getSize()).height)) / 2) + ((parentWindow
						.getLocation()).y)));
	...
	private void floatText() {
		parentWindow.canvas.draftText = new OrbitText(
				parentWindow.canvas.drawColor, parentWindow.canvas.oldx,
				parentWindow.canvas.oldy, parentWindow.drawFontNumber,
				parentWindow.drawStyle, parentWindow.drawSize, text.getText());
	...
	public void itemStateChanged(ItemEvent E) {
		if (E.getSource() == bold) {
			...
			parentWindow.drawSize = Integer.parseInt(size.getSelectedItem());
			sample.repaint();
			return;
		}
	}
	...
	public void keyReleased(KeyEvent E) {
		if (E.getKeyCode() == E.VK_ENTER) {
			if ((E.getSource() == ok) || (E.getSource() == text)) {
				floatText();
				dispose();
				return;
			}
...
public class OrbitCanvas extends Canvas implements MouseListener,
		MouseMotionListener {
	...
	public void floatPicture(File pictureFile) {
		...
		draftPicture = new OrbitPicture(10, 10, theImage.getWidth(this),
				theImage.getHeight(this), pictureFile, theImage);

		// Draw the initial draft rectangle
		drawRect(Color.black, draftPicture.x, draftPicture.y,
				draftPicture.width, draftPicture.height, false, 2,
				this.MODE_XOR);
	...
	synchronized public void drawLine(Color color, int startx, int starty,
			int endx, int endy, int thickness, int mode) {
		int dx, dy;
	...
	synchronized public void drawText(Color color, int x, int y,
		...
		g1.setFont(new Font(OrbitCanvas.fontArray[fontnumber], attribs, size));
		g2.setFont(new Font(OrbitCanvas.fontArray[fontnumber], attribs, size));
	...
	public void mousePressed(MouseEvent E) {
		...
		if (drawType == this.LINE) {
			draftLine = new OrbitLine(drawColor, oldx, oldy, oldx, oldy,
					drawThickness);
			...
			draftOval = new OrbitOval(drawColor, oldx, oldy, 1, 1, false,
					drawThickness);

			// Set the draw mode to XOR and draw it.
			drawOval(draftOval.color, draftOval.x, draftOval.y,
					draftOval.width, draftOval.height, false, drawThickness,
					this.MODE_XOR);
		}

		else if (drawType == this.RECTANGLE) {
			draftRect = new OrbitRectangle(drawColor, oldx, oldy, 1, 1, false,
					drawThickness);

			// Set the draw mode to XOR and draw it.
			drawRect(draftRect.color, draftRect.x, draftRect.y,
					draftRect.width, draftRect.height, false, drawThickness,
					this.MODE_XOR);
		}

		// Set the activity message
		if (!parentWindow.activity.getText().equals(
				"drawing: " + parentWindow.name))
			parentWindow.activity.setText("drawing: " + parentWindow.name);

		if (parentWindow.connected)
			try {
				parentWindow.theClient
						.sendActivity(OrbitCommand.ACTIVITY_DRAWING);
			} catch (IOException e) {
	...
	public void mouseReleased(MouseEvent E) {
		if (floatingPicture) {
			...
			drawPicture(draftPicture.x, draftPicture.y, draftPicture.picture);

			// Send the image to the socket
			if (parentWindow.connected)
				try {
					parentWindow.theClient.sendDrawPicture(
							(short) draftPicture.x, (short) draftPicture.y,
							draftPicture.file);
				} catch (IOException e) {
					parentWindow.theClient.lostConnection();
					return;
			...
			drawLine(drawColor, oldx, oldy, E.getX(), E.getY(), drawThickness,
					this.MODE_PAINT);
			...
			if (parentWindow.connected)
				try {
					parentWindow.theClient
							.sendLine((short) parentWindow.colorChoice
									.getSelectedIndex(), (short) oldx,
									(short) oldy, (short) E.getX(), (short) E
											.getY(), (short) drawThickness);
				} catch (IOException e) {
					parentWindow.theClient.lostConnection();
					return;
				}
			...
			drawOval(drawColor, oldx, oldy, ovalWidth, ovalHeight, fill,
					drawThickness, this.MODE_PAINT);

			if (parentWindow.connected)
				try {
					parentWindow.theClient
							.sendPoly((short) parentWindow.colorChoice
									.getSelectedIndex(), (short) oldx,
									(short) oldy, (short) ovalWidth,
			...
			drawRect(drawColor, oldx, oldy, rectangleWidth, rectangleHeight,
			...
			if (parentWindow.connected)
				try {
					parentWindow.theClient
							.sendPoly((short) parentWindow.colorChoice
									.getSelectedIndex(), (short) oldx,
									(short) oldy, (short) rectangleWidth,
			...
			if (floatingText) {
				...
				drawText(drawColor, draftText.x, draftText.y,
						draftText.fontnumber, draftText.attribs,
						draftText.size, draftText.text, this.MODE_PAINT);

				// Output to the other clients
				if (parentWindow.connected)
					try {
						parentWindow.theClient.sendDrawText(
								(short) parentWindow.colorChoice
										.getSelectedIndex(),
								(short) draftText.x, (short) draftText.y,
								(short) draftText.fontnumber,
								(short) draftText.attribs,
								(short) draftText.size, draftText.text);
					} catch (IOException e) {
						parentWindow.theClient.lostConnection();
						return;
					}
	...
	public void mouseMoved(MouseEvent E) {
		if (floatingText) {
			...
			// Erase the old draft text
			drawText(drawColor, draftText.x, draftText.y, draftText.fontnumber,
					draftText.attribs, draftText.size, draftText.text,
					this.MODE_XOR);
