...
public class OrbitRoomControl extends Dialog implements ActionListener,
	...
	OrbitRoomControl(Frame parent) {
		...
		setSize(200, 200);
		pack();
		setLocation(parentWindow.getLocation().x, parentWindow.getLocation().y);
	...
	private boolean userInRoom(String userName, OrbitRoomInfo room) {
		for (int count = 0; count < room.userNames.size(); count++) {
			if (((String) room.userNames.elementAt(count)).equals(userName))
				return (true);
		}
	...
	public void updateLists() {
		...
		for (int count = 0; count < currentRoom.userNames.size(); count++)
			roomUsersList.add((String) currentRoom.userNames.elementAt(count));
	...
	protected void goBootUsers() {
		...
		for (int count1 = 0; count1 < selectedUsers.length; count1++) {
			...
			for (int count2 = 0; count2 < usersVector.size(); count2++) {
				OrbitUser user = (OrbitUser) usersVector.elementAt(count2);
	...
	public void itemStateChanged(ItemEvent E) {
		if (E.getSource() == roomUsersList) {
			boolean somethingSelected = (roomUsersList.getSelectedItems().length > 0);
			...
			boolean somethingSelected = (allUsersList.getSelectedItems().length > 0);
