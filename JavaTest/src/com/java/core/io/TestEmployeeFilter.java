...
public class TestEmployeeFilter {
	...
	public static void employeeFilter(){
		...
		switch (mainInput) {
			...
			case 2:
				System.out.println("Please select ..");
				System.out.println("1. Age filter \n2. Name filter \n3. Designation filter");
				...
				switch (subInput2) {
				case 1:
					System.out.println("Please enter Employee Age and filters like ..");
					...
					tempEemployeesList = filterEmployeesAgeOnCondition(age, condition1, employeesList);
				...
				case 2:
					System.out.println("Please enter Employee Name(Pattern) and filters like ..");
					System.out.println("1. Whole String \n2. Starts with \n3. Ends with \n4. Contains char set");
					...
					try {
						...
						tempEemployeesList = getPatternedEmployeeNames(patternCondition, pattern, condition, employeesList);
				...
				case 3:
					System.out.println("Please enter Employee Designation and filters like ..");
					System.out.println("1. < \n2. > ");
					...
					try {
						...
						tempEemployeesList = filterEmployeesDesigOnCondition(designation,condition,employeesList);
	 ...
	 */
	private static List<Employee> getPatternedEmployeeNames(int patternCondition,String pattern,int condition,List<Employee> employeesList){
	 ...
	 * @return List<Employee>
	 */
	private static List<Employee> filterEmployeesDesigOnCondition(String designation,int condition,List<Employee> employeesList){
	...
	/**
	 * returns a employee list with designation values 
	 * @return List<String>
	 */
	 ...
	 * @return List<Employee>
	 */
	private static List<Employee> filterEmployeesAgeOnCondition(int age,int condition,List<Employee> employeesList){
	 ...
	 */
	private static Map<String, Integer> getDesignationEmployees(List<Employee> employeesList){
	...
	/**
	 * returns employees list which is filtered by start age and end age of employees 
	 * @param employeesList
	 * @param startAge
	 * @param endAge
	 * @return Map<String, Integer>
	 */
	private static Map<String, Integer> getAgeEmployees(List<Employee> employeesList, int startAge, int endAge){
		int range = 0;
	...
	/**
	 * Prints employees when a list passed as a parameter and with some conditions
	 * @param employeesList
	 * @param mainCond
	 * @param subCond
	 */
	private static void printEmployeeList(List<Employee> employeesList,int mainCond, int subCond){
		...
		case 2:
			if(subCond == 1)
				...
				Collections.sort(employeesList, new MyComparator("reverseName"));
			printConditionWiseEmployees(employeesList);
			break;
			
		case 3:
			if(subCond == 1)
				Collections.sort(employeesList, new MyComparator("designation"));
			else if(subCond == 2)
				Collections.sort(employeesList, new MyComparator("reverseDesignation"));
			printConditionWiseEmployees(employeesList);		
			break;
	...
	private static void printConditionWiseEmployees(List<Employee> employeesList){
		...
		if(!employeesList.isEmpty()){
			for (Iterator<Employee> iterator = employeesList.iterator(); iterator.hasNext();) {
				...
				System.out.println(employee.name.trim()+"\t\t\t"+employee.designation.trim()+"\t\t\t"+employee.age);
			}
		}else
			System.out.println("No data matching");
	}
	...
	/**
	 * returns a employee list with randomly created names, age and designations
	 * @return List<Employee>
	 */
	private static List<Employee> createEmployees(){
		...
		Random randomGenerator = new Random();
		    for (int idx = 1; idx <= 10; ++idx){
		      ...
		      do{
		    	  randomIndex = randomGenerator.nextInt(getEmployeeDesigLevel().size());
		    	  System.out.println("randomIndex "+randomIndex);
		      }while(randomIndex > 21);
		      
		      Employee employee = new Employee(getString(10), getEmployeeDesigLevel().get(randomIndex), randomAge);
...
/**
 * Employee class which has name, age and designation as properties
...
/**
 * it is a utility class to compare employee objects based on different prperties
 * @author srinath.rayabarapu
 */
class MyComparator implements Comparator<Employee> {
	
	String comp = "age";
	MyComparator(String param) {comp = param;}
	MyComparator() {}
