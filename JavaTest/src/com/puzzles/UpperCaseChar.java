...
/* Write a method that returns the number of uppercase letters in a string object passed to a method as a 
 * parameter. Use the class method isUpperCase( ) of the Character class, which returns true if the passed 
 * parameter of type char is an uppercase letter. You need explore the Character class from the java.lang 
 * package on your own. 
 */
...
class Count {
	...
	// counter variable is used to store the no.of uppercase letter found in the
	// string

	int counter = 0;

	// checkUpperCase is a method that accepts the string object as a parameter
	// and returns the total no.of uppercase character within the string

	int checkUpperCase(String str) {

		// str.length() method returns the length of the string
		// variable i is used for iterations.
		// for every iteration it takes a single character from the string and
		// checks for the uppercase character.

		// charAt() method returns a character at specified position

		// isUpperCase() method returns a boolean if the character is in
		// uppercase

		for (int i = 0; i < str.length(); i++) {
			if ((isUpperCase(str.charAt(i))) == true)
				counter++; // counter gets incremented every time the condition
							// becomes true
		} // end of for loop
...
public class UpperCaseChar {
	public static void main(String[] args) {
		...
		// num_of_chars variable is used to get the no.of characters in
		// uppercase returned by the Count class method
