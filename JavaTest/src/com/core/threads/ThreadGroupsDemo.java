	...
	public void run() {
		for (int i = 0; i < 30; i++) {
			try {
				...
				// threads, we can have control over all the threads of the
				// group like this
				// dangerous getThreadGroup().suspend(); // stop the execution
				// of threads for a while
				r[i].start();
...
class Reservation extends Thread {
	public void run() {
		try {
			System.out
					.println(" I am reservation thread and my priority is set to "
							+ getPriority());
			sleep(300);
		} catch (Exception e) {
		}
