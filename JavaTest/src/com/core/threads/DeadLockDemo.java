...
class DeadLockDemo {
	static public void main(String[] args) throws Exception {
		...
		Object compartment = new Object();// this has to be assumed as a
											// compartment
...
class CancelTicket extends Thread {
	...
	public void run() {
		...
		synchronized (comp) {
			...
			// if cancel ticket is performed very quickely we may not face
			// deadlock,
			// try removing sleep and see. This is why we say thread programs
			// are difficult to
			// debug .In many cases due to problems with threads whole system
			// hangs.
			System.out
					.println("  cancel ticket  now trying to grab lock on train");
			synchronized (train) {
...
class BookTicket extends Thread {
	...
	public void run() {
		...
		synchronized (train) {
			...
			System.out
					.println("  book ticket   now trying to grab lock on comp");
			synchronized (comp) {
