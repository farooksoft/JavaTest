...
class BookTicket1 extends Thread {
	...
	public void run() {
		...
		synchronized (comp) {
			...
			System.out
					.println("  book ticket   now trying to grab lock on train");
			synchronized (train) {
...
// It is better to obtain locks in the same sequence to avoid deadlocks.
// Deadlocks
 ...
 * 
 * wait for some time oracle server detects deadlock situation and notifies u
 * about deadlocks
 */
