...
class SortOne {
	...
	// temp_str is a string array and temp is a string varaible which is
	// initialsied as null
	...
	// wordSort is a method which consists of hasMoreElements to seperate the
	// given string
	void wordSort() {
		// tokenizer is pointing to the "this is a book" string.
		tokenizer = new StringTokenizer(str);

		// counting the total number of words within the given string and
		// storing it in words variable
		words = tokenizer.countTokens();

		// puting that total number of words inside the array variable temp_str.
		temp_str = new String[words];

		// arranging the letters / names in alphabetical order using
		// hasMoreElements and nextToken methods.
		...
		for (int i = 0; i < words; i++) {
			...
			// comparing between both the words. if it is greater than zero then
			// arranges those words in alphabetical order.
